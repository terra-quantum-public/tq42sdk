.. TQ42SDK documentation master file, created by
   sphinx-quickstart on Thu Jun 15 10:57:58 2023.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

Welcome to TQ42SDK's Documentation
==================================

.. toctree::
   :maxdepth: 4
   :caption: Introduction:

   README.md
   Quick_Start.md
   Help_Center.md

.. toctree::
   :maxdepth: 4
   :caption: Python Developer Guide:

   Python_Developer_Guide/Introduction.md
   Python_Developer_Guide/Help.md
   Python_Developer_Guide/Command_Structure.md
   Python_Developer_Guide/Setting_Up_Your_Environment.md
   Python_Developer_Guide/Selecting_Compute_Resources.md
   Python_Developer_Guide/Working_with_Datasets.md
   Python_Developer_Guide/Working_with_Models.md
   Python_Developer_Guide/Preparing_Your_Data_and_Hyper_Parameters.md
   Python_Developer_Guide/Submitting_and_Monitoring_a_Run.md
   Python_Developer_Guide/Errors.md

.. toctree::
   :maxdepth: 4
   :caption: CLI Developer Guide:

   CLI_Developer_Guide/Introduction.md
   CLI_Developer_Guide/Help.md
   CLI_Developer_Guide/Command_Structure.md
   CLI_Developer_Guide/Setting_Up_Your_Environment.md
   CLI_Developer_Guide/Selecting_Compute_Resources.md
   CLI_Developer_Guide/Working_with_Datasets.md
   CLI_Developer_Guide/Working_with_Models.md
   CLI_Developer_Guide/Preparing_Your_Data_and_Hyper_Parameters.md
   CLI_Developer_Guide/Submitting_and_Monitoring_a_Run.md
   CLI_Developer_Guide/Errors.md

.. toctree::
   :maxdepth: 4
   :caption: Dataset Creation:

   Dataset_creation/Dataset_creation.md

.. toctree::
   :maxdepth: 4
   :caption: Objective Functions:

   Objective_Functions/Objective_Function.md

.. toctree::
   :maxdepth: 4
   :caption: TetraOpt Algorithm:

   TetraOpt_Algorithm/Introduction.md
   TetraOpt_Algorithm/Parameters_and_Hyperparameters.md
   TetraOpt_Algorithm/Output_Format.md
   TetraOpt_Algorithm/Assumptions_Limitations.md
   TetraOpt_Algorithm/Best_Use_Cases_Applicability.md

.. toctree::
   :maxdepth: 4
   :caption: CVAOpt Algorithm:

   CVAOpt_Algorithm/Introduction.md
   CVAOpt_Algorithm/Parameters_and_Hyperparameters.md
   CVAOpt_Algorithm/Output_Format.md

.. toctree::
   :maxdepth: 4
   :caption: QuEnc Algorithm:

   QuEnc_Algorithm/Introduction.md
   QuEnc_Algorithm/Parameters_and_Hyperparameters.md
   QuEnc_Algorithm/Example_Problem.md


Indices and tables
==================

* :ref:`genindex`
* :ref:`modindex`
